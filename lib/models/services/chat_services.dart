...
class ChatServices {
  static getVendorChats(Function callback, token, vendorId) async {
    var response = await ApiServices.initialiseGetRequest(
        url: '$getVendorChatUrl/$vendorId/chat', token: token);
  ...
  static getChatsById(Function callback, token, userId) async {
    var response = await ApiServices.initialiseGetRequest(
