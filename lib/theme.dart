import 'dart:ui';

const Color primary = Color(0xFF31456A);
const Color dark = Color(0xFF334669);
const Color textColor = Color(0xFF31456A);
const Color white = Color(0xFFFFFFFF);
const Color grey = Color(0xFF5B80A3);
const Color black = Color(0xFF000000);
const Color blue = Color(0xFF1994F5);
const Color amber = Color(0xFFFFC43A);
// #FEF7E2
const Color lightGrey = Color(0xFF8F9FAE);
const Color light = Color(0xFFB3B9C9);
const Color peach = Color(0xFFEE9378);
const Color green = Color(0xFF1DF489);
const Color lightGreen = Color(0xFF28D7AD);
const Color darkgreen = Color(0xFF0F7D46);
const Color background =  Color(0xFFE4F0FA);
const Color red = Color(0xFFF1004B);

 Color iconGrey65 = const Color(0xFf334669).withOpacity(0.6);
 Color iconGrey75 = const Color(0xFf334669).withOpacity(0.75);
 Color iconGrey = const Color(0xFf334669);
